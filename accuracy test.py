...
def get_band_name(balance: float) -> str | None:
    match = bands.loc[(bands["Minimum"] <= balance) & (bands["Maximum"] >= balance)]
    if not match.empty:
...
def get_rate_for_date_and_band(date: pd.Timestamp, band_name: str) -> float:
    applicable_rates = rates.loc[(rates["Start Date"] <= date) & (rates["band"] == band_name)]
...
def calculate_interest_with_steps(transactions_df: pd.DataFrame, end_date: datetime):
    ...
    for current_date in date_range:
        ...
        # Round balance to 2 decimals to simulate actual ledger precision
        ...
        if band is None:
            ...
            daily_interest = round(balance * rate, 6)  # rounding daily interest for precision
...
if uploaded_file:
    try:
        ...
        ledger = df_raw.loc[:, ["Date", "Client Amount"]].copy()
