...
if uploaded_file:
    try:
        # Read CSV with special options to handle multiline and quoting issues
        df_raw = pd.read_csv(
            uploaded_file,
            skiprows=5,
            engine='python',
        ...
        # Clean columns: strip whitespace and remove newlines for consistent naming
        df_raw.columns = [col.strip().replace('\n', ' ').replace('\r', '') for col in df_raw.columns]

        # Show detected columns for debugging
        st.write("Detected columns:", df_raw.columns.tolist())

        # Find columns containing 'Date' and 'Client' (partial match)
        ...
        # Extract numeric values from "Client" column, removing currency symbols and commas
        ledger["Change"] = ledger["Client"].astype(str).str.extract(r'(-?[\d,.]+)')[0]
        ledger["Change"] = ledger["Change"].str.replace(',', '')
        ledger["Change"] = pd.to_numeric(ledger["Change"], errors='coerce')

        # Drop rows where Date or Change is missing (NaN)
